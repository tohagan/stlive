...
module.exports = function(options, args) {
    ...
    ["JAVA_HOME", "ANT_HOME", "ANDROID_HOME"].forEach(function(v) {
        if (!process.env[v]) {
            throw '$' + v + ': Required environment variable is not set';
        }
    });
    ...
    if (options.live) {
        ...
        require('./live').addLive();
    }

    runcmd (options.senchaCmd + ' app build native');

    var thisApp = path.basename(process.argv[1]);
    console.log(("Completed building your '" + options.appName + "' app.").lightYellow);
    ...
    console.log(("$ " + thisApp + " serve").lightGreen);

    console.log('config.orig.xml: Used when "live" is disabled'.lightYellow);
    console.log('config.live.xml: Used when "live" is enabled'.lightYellow);
