        ...
        server.listen(options)
            .on('error', function(err) {
                if (err.code === 'EADDRINUSE') {
                    ...
                    console.log(("--port port option OR set port in ~/.stlive.json").lightGreen);
                } else {
            ...
            .on('log', function(msg, url, staticpath) {
                if (url == '/__api__/autoreload') {
                    ...
                    // Temporary fix until bug in connect-phonegap is fixed.  Should have fired an error message.
                    // If getAddressInfo() call fails, URL contains  "unknown" in URL as hostname when host has no network connection.
                    if (msg === 'listening on' && url.indexOf('unknown:') === 0) {
                        console.log("Failed to find IP address of this computer.  Is your network connected?".lightRed);
                        process.exit(1);
                    }
