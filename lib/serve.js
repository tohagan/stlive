...
function addServerOptions(options) {
    ...
    if (dirs.isSencha()) {
        ...
        options.sass = false;  // SASS Compiler is only auto started for Sencha projects
    } else {
...
function startBgTasks(options, cb) {
    if (options.sass) {
        ...
        if (options.sass) bgTask('sass', cb, options);
    } else {
        cb();
    }
}
        ...
        server.listen(options)
            ...
            .on('log', function(statusCode, url, staticpath) {
                if (url == '/__api__/autoreload') {
                    if (options.debug) process.stdout.write('\033[0GReloads: ' + ('' + reloads++).lightGreen);
                } else {
