...
//        console.info("or run 'stlive diff [file]' to compare defaults.json with a settings file or ~/.stlive.json"); 
//    }
//}

module.exports = {
    show: function(options, args) {
        console.info((options._pkg.name + " " + options._pkg.version).lightYellow); 
        ...
        console.info('');        
        var configFile = (process.platform === "win32") ? '%USERPROFILE%\\.stlive.json' : '~/.stlive.json';
        console.info(("Edit " + configFile + " to configure app settings.").lightYellow); 
    },
    
    diff: function(options, args) {
        var homeFile = expandHome('~/.stlive.json');
        var settingsFile = (args.length > 0 && args[0]) || homeFile;
        var defaultsFile = path.join(__dirname, '..', '.stlive.json');
